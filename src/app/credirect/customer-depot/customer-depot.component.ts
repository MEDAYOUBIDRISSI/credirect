...
export class CustomerDepotComponent implements OnInit{
  ...
  getDossiersByDepots(){
    ...
    this.CustomerService.getDossiersByDepots(body).then((res: any) => {
      console.log("res", res);
      if (res.status_code === 200) {
        this.dossiers = res.data.map((dossier: any) => {
          const dossierId = dossier?.creditId;
          const depotId = dossier?.depotId; // OR get the real depot ID if available
          return {
            ...dossier,
            panelMenuItems: this.getPanelMenuItems(dossierId, depotId)
          };
        });
